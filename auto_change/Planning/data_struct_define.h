...
#define SAFETY_GAP_DISTANCE 0.5
...
#define MAP_MAX_DISTANCE 3.0

// Planning Module Param Struct
typedef struct Struct_PlanningConfig
{
	double fSafeDis1;									// 最小安全距离（纵向）
	double fSafeDis2;									// 中等安全距离（纵向）
	double fMaxPlanningDistance;						// 最大规划距离
	double fMaxAcc;										// 最大加速度
	double fMaxV;										// 最大速度
	double follow_dis;									// 跟车距离
	double fUltrasonicSpeed;
	double fUltrasonicDis;
	double fVehicleWidth;								// 车身宽度
	double fVehicleLength;								// 车身长度
	...
	double fLeftHalfRoadWidth;							// 左侧道路宽度默认值
	double fRightHalfRoadWidth;							// 右侧道路宽度默认值
	double dOriginLat;									// ENU坐标系原点定义
	double dOriginLon;
	double dOriginAlt;
	int    iRecordFile;
	bool   ReadPlanningConfig;
}StrPlanningConfig;
...
typedef struct Struct_ControllingConfig
{
	...
	bool   ReadControlConfig;							// Read control configuration file successfully
...
// 高精地图数据结构体，XYZ坐标是在ENU坐标系下的坐标，ENU坐标系的原点是地图的起始点
...
typedef struct _Struct_SL_Points
{
	int iRoadType;
	int iNum;
	int iLightStatus;									// 转向灯状态：0为直行，1为左转，-1为右转
	int iSpoutWater;									// 喷水状态：0为停止喷水，1为启动喷水
	int iCleanStatus;									// 清扫状态：0为停止清扫，1为启动清扫
	int iCleanAsh;										// 清灰状态：0为停止清灰，1为启动清灰
	int iStatus;										// -2为定位无效，-1为异常状态，规划失败，0为正常规划状态，1为超声波雷达有效，3为停车
	double pfC[4];
...
typedef struct _Struct_Target_Fusion
{
	unsigned long long ullTimestamp;					// 时间戳
	int iTargetNum;										// 目标个数
	StrTargetStatus strTargetStatus[MAX_TARGET_NUM];	// 障碍物目标状态信息（只包含毫米波和激光雷达的障碍物数据，不包含超声波的障碍物数据）
	StrRodeBoard strRoadBoard;							// 道路边界信息
	StrUltrasonicDatas strUltrasonicDatas;				// 超声波信息，这里把超声波的障碍物信息单独分开出来了
}StrTargetFusion;
