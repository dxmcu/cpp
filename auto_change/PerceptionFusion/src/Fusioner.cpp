...
bool CFusioner::ReceivePerceptionData()
{
	...
	{
		//解析失败
		cout << "zmq perception json parse failed" << endl;
		cout << "receive: " << str_recv.c_str() << endl;
		return false;
	}
	string type = updated_perception_value["TYPE"].asString();
	...
	{
		cout << "returned type is not qualified for perception fusion！" << endl;
		return false;
	}
	return true;
}
...
bool CFusioner::SendFusedResult()
{
	...
	string str_send = writer.write(rep);
...
void CFusioner::ReadLocalData(string path, int dataid)
{
	string str_sense; 
	string str_gps;
	string filepath1 = path + std::to_string(dataid) + "_sense.txt";
	string filepath2 = path + std::to_string(dataid) + "_gps.txt";
	fstream f1, f2;
	Json::CharReaderBuilder rbuilder;
	JSONCPP_STRING errs;


	f1.open(filepath1.c_str(), ios::in);
	bool parse_ok = Json::parseFromStream(rbuilder, f1, &recv_sense_, &errs);
	if (!parse_ok)
	{
		recv_sense_.clear();
		cout << "Parse json sense file error!" << endl;
	}
	f1.close();

	f2.open(filepath2.c_str(), ios::in);
	parse_ok = Json::parseFromStream(rbuilder, f2, &recv_gps_, &errs);
	if (!parse_ok)
	{
		recv_sense_.clear();
		cout << "Parse json gps file error!" << endl;
	}
	f2.close();
