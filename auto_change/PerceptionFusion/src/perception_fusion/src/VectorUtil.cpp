...
int DeepBlue::CVectorUtil::TwoLineSegmentsRelDetect(double px1, double py1, double px2, double py2, double qx1, double qy1, double qx2, double qy2, GeomPoint& pt)
{
  ...
  if (std::min(px1, px2) > std::max(qx1, qx2) || std::max(px1, px2) < std::min(qx1, qx2))
    return result;
  if (std::min(py1, py2) > std::max(qy1, qy2) || std::max(py1, py2) < std::min(qy1, qy2))
    return result;
...
{
  if ((fabs(std::min(px1, px2) - std::min(qx1, qx2))<dbl_precesion) &&
      (fabs(std::max(px1, px2) - std::max(qx1, qx2))<dbl_precesion) &&
      (fabs(std::min(py1, py2) - std::min(qy1, qy2))<dbl_precesion) &&
      (fabs(std::max(py1, py2) - std::max(qy1, qy2))<dbl_precesion))
    return true;
