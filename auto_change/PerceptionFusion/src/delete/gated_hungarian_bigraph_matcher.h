	...
	namespace perception {
		namespace common {
			...
			public:
				...
				* and update it completly is STRONG RECOMMENDED!! P.S. resizing LossMat
				* would not alloc new memory, if the resizing size is smaller than the
				* size reserved. */
				const LossMat<T>& global_costs() const { return global_costs_; }
				LossMat<T>* mutable_global_costs() { return &global_costs_; }
			...
			void GatedHungarianMatcher<T>::UpdateGatingLocalCostsMat(
				...
				LossMat<T>* local_costs = optimizer_.costs();
